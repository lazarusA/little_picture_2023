...
function circleShape(h,k,r; repeat = 10)
    θ = range(0, 2π, 24*repeat)
...
for (c, city) in enumerate(cities)
    scity = scities[c]
    f10_prints = open_dataset(joinpath(@__DIR__, "./data/tp_$(scity).zarr/"))
    ...
    let
        for bgc in [:white, :black]
            ...
            for (k,i) in enumerate(range(0.25,2,20))
                lines!(ax, circleShape(0,0,i); color = repeat(f10_prints[:,k] .+1e-10, inner=10),
            ...
            text!(ax_title, Point2f(0,-0.85),
                text = rich("Mean total rainfall by hour\n\n\n", fontsize = 64, font=:bold,
                    ...
                    rich("What hour of the day has been the most rainy? This little picture illustrates\n\nthe mean total rainfall by hour per year and across various years in\n\n", fontsize = 38, font=:regular), rich(city, font=:bold, fontsize=38)),
                align = (:left, :center),
                color = :black)   
            ...
            colgap!(fig.layout, 0)
            rowgap!(fig.layout, 0)
            save("./little_pictures/$(bgc)/rainfall_by_hour_$(scity).png", fig)
        end
    end
end